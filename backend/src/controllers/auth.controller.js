...
export const login = async (req, res)=>{
    ...
    try{
        const user = await User.findOne({email});
        // if user is already not present in the DB
        if(!user){
            return res.status(400).json({message: "Invalid credentials"});
        }

        // if email found in the DB then compare its password with stored password for authenticity
        const isPasswordCorrect = await bcrypt.compare(password, user.password);
        if(!isPasswordCorrect){
           return res.status(400).json({message: "Invalid credentials"}); 
        }

        // if email and password staisfy their cond
...
export const logout = (req, res)=>{
    // to logout just clear the cookie it by default get log out
    try {
...
// It just give authenticated user
